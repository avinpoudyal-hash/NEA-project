...
class newCircle {
    constructor(x_pos, y_pos, speed, elasticity, gravity, mass, friction, restitution, distanceTravelled){   //constructor function to set up the circle
    this.x_pos = x_pos
    this.y_pos = y_pos
    this.speed = speed
    ...
    drawNew(context){   //function to draw the circle
        ...
        context.arc(this.x_pos, this.y_pos, 30, 0, Math.PI * 2, false)  //draws circle at x_pos, y_pos with radius 30
        context.fillStyle = 'red';
        context.fill();
    ...
    movement(context){  //function to move the circle
        if (this.y_pos + 30 >= 655) {
            this.distanceTravelled = this.distanceTravelled / 60
            this.Time = this.Time / 60
            if (this.alerted == false){
                alert("The circle has hit the ground and stopped moving and has travelled a distance of " + this.distanceTravelled.toFixed(2) + " metres in " + this.Time.toFixed(2) + " seconds.");
                this.alerted = true
            }
            this.speed = 0
            this.gravity = 0
...
let draw_circle = new newCircle(x_pos, y_pos, speed, elasticity, gravity, mass, friction, restitution, distanceTravelled)    //creates a new circle object
